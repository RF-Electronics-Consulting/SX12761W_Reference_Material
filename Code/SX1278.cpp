...
int16_t SX1278::readRegister(uint16_t registerToRead)
{
  int16_t result = 0;

  Module* mod = this->getMod();

  result = mod->SPIgetRegValue(registerToRead);
...
int16_t SX1278::setBandwidthRaw(uint8_t newBandwidth) {
  ...
  // // Turn LnaBoost On
  // state |= mod->SPIsetRegValue(0x0C,3,1,0);
  
  // if(RADIOLIB_SX1278_BW_500_00_KHZ == newBandwidth)
  // {
  //   // Sensitivity Optimization per Semtech ErrataNote
  //   state |= mod->SPIsetRegValue(0x36, 0x02, 7, 0);
  //   state |= mod->SPIsetRegValue(0x3A, 0x64, 7, 0);
  // }
...
void SX1278::errataFix(bool rx) {
  ...
  if(fabs(SX127x::bandwidth - 500.0) <= 0.001) {
    if((frequency >= 862.0) && (frequency <= 1020.0)) {
      ...
      Serial.println("**************Optimized for 915MHz");
    } else if((frequency >= 410.0) && (frequency <= 525.0)) {
      mod->SPIwriteRegister(0x36, 0x02);
      mod->SPIwriteRegister(0x3a, 0x7F);
      Serial.println("**************Optimized for 450MHz");
    }
  }
  ...
  if(fabs(SX127x::bandwidth - 7.8) <= 0.001) {
    ...
    Serial.println("***********Bandwidth not found");
    return;
  }
